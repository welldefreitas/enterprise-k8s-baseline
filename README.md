...
- Deletion protection option for prod safety guardrails
...
## Remote state (GCS) per environment
This repo is designed for **remote state by default** (GCS), separated by environment.

1) Create state buckets (versioning + UBLA):
```bash
...
2) Update the bucket names in:
- `envs/gcp/dev/backend.hcl`
- `envs/gcp/prod/backend.hcl`
...
> Security guardrail: if `enable_private_endpoint=false`, you must provide `allowed_admin_cidrs` (control plane allowlist).
...
## Policy-as-code
Conftest/OPA baseline checks (no cloud credentials required):
```bash
make policy
```
...
## GitHub Actions OIDC (Plan vs Apply)
Recommended split identities: read-only `tf-plan` vs write `tf-apply`.
See `docs/06-ci-identities.md` and `scripts/setup_ci_gcp.sh`.

### Auditor-grade Apply
This repo includes a "Plan then Apply" workflow with **manual approval** via GitHub Environments.
See:
- `docs/07-auditor-grade-apply.md`
- `docs/09-branch-protection.md`
